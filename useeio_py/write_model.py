...
# The list of matrices to write out
matrices = ["V", "U", "U_d", "A", "A_d", "B", "C", "D", "L",
            "L_d", "M", "M_d", "N", "N_d", "Rho", "Phi"]


def write_model_for_api(model, base_dir):
    '''
...
def write_model_matrices(model, to_format, output_folder):
    '''
...
def write_model_to_xlsx(model, output_folder):
    '''
...
def generate_model_identifier(model):
    '''
...
def set_write_dirs(base_dir, model = None):
    '''
...
def prepare_write_dirs(model, dirs):
    '''
    #' Sets directories to write model output data to
    #' @param model A complete EEIO model: a list with USEEIO model components and attributes.
...
def write_model_demand_to_json(model, output_folder):
    '''
...
def write_model_metadata(model, dirs):
    '''
...
def write_sector_crosswalk(model, output_folder):
    '''
...
def write_session_info_to_file(path):
    '''
...
def prepare_model_sector_crosswalk(model):
    '''
