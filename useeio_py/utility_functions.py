...
def join_strings_with_slashes(s):
    '''
...
def calculate_output_ratio(model, output_type = "Commodity"):
    '''
...
def compare_matrices(m1, m2, percentage_diff = False):
    '''
...
def write_matrix_as_bin_file(matrix, path):
    '''
...
def download_data_commons_file(source, subdirectory, debug_url):
    '''
...
def load_data_commons_file(static_file):
    '''
...
def map_fips5_to_location_names(fips_codes, fips_system):
    '''
...
def map_location_codes_to_names(codes, code_system):
    '''
...
def ras(m0, t_r, t_c, t, max_itr = 1E6):
    '''
...
def apply_ras(m0, t_r, t_c, relative_diff, absolute_diff, max_itr):
    '''
...
def convert_str_encoding_lat_into_ascii(s):
    '''
...
def write_metadata_to_json(package, name, year, source, url, date_last_modified, date_accessed):
    '''
...
def format_location_for_state_models(location):
    '''
