...
# or use plotnine to allow ggplot calls in python


def plot_matrix_coefficient(model_list, matrix_name, coefficient_name, sector_to_remove, y_title, y_label, log_scale = False):
    '''
...
def barplot_indicator_scores_by_sector(model_list, totals_by_sector_name, indicator_name, sector, y_title):
    '''
...
def heatmap_satellite_table_coverage(model, form = "Commodity"):
    '''
...
def heatmap_sector_ranking(model, matrix, indicators, sector_to_remove, N_sector, x_title = None):
    '''
...
def barplot_flow_or_impact_fraction_by_region(R1_calc_result, Total_calc_result, x_title):
    '''
...
def get_bea_sector_color_mapping(model):
    '''
...
def get_indicator_color_mapping():
    '''
